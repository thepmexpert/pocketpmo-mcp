 ...
 * absolute paths never cross this boundary (leak rule: full paths
 * survive only in fatal config errors the operator reads).
 */
function forEachEntry(onEntry) {
  ...
  try {
    ...
    onEntry(null, `projects dir not readable: ${dir}`);
    return;
  }
...
export function getProject(idOrName) {
  ...
  if (!matched && !validIds.length) {
    ...
    // MCP log channel. Guarded: a broken stderr must not turn a diagnostic
    // into a crash.
    try {
      process.stderr.write(`pocketpmo-mcp: projects directory is empty: ${projectsDir()}\n`);
    } catch {}
