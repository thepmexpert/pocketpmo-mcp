 ...
 * All results rounded to 2dp. Invalid input -> NaN (never throws): the
 * app's UI (PERTAnalysis saveEditMode) rejects estimates that are negative
 * or violate o <= m <= p before they are stored, so this guard mirrors that
 * data contract defensively — reversed/negative triples get NaN like any
 * other unusable input, never a silently wrong estimate.
 */
...
export function validateActivities(activities) {
  ...
  for (const a of activities) {
    ...
    if (d) {
      const o = d.optimistic ?? Math.max(1, (validDuration(a.duration) ? a.duration : 1) * 0.7);
      const m = d.mostLikely ?? (validDuration(a.duration) ? a.duration : 1);
      const pe = d.pessimistic ?? (validDuration(a.duration) ? a.duration : 1) * 1.5;
      if (!validPertOrdering(o, m, pe)) {
        issues.push({
          activityId: a.id,
          field: 'distribution',
          message: `pert estimates violate optimistic <= mostLikely <= pessimistic (${o} > ${m} > ${pe} when inverted); triangular sampling produces negative durations`,
          received: { optimistic: d.optimistic ?? null, mostLikely: d.mostLikely ?? null, pessimistic: d.pessimistic ?? null }
        });
      }
    }
