...
// Bounded diagnostic context for error strings (cubic P3, PR #14): a dir
// with many malformed files must not inflate a single error line — the
// first few basenames go inline, the overflow points at list_projects
// (warnings[]), which always carries the full sanitized list.
const MAX_ERROR_WARNINGS = 3;

function joinWarningsBounded(warnings) {
  const shown = warnings.slice(0, MAX_ERROR_WARNINGS).join('; ');
  const rest = warnings.length - MAX_ERROR_WARNINGS;
...
export function getProject(idOrName) {
  ...
  if (!matched && !validIds.length) {
    const detail = warnings.length ? ` (${joinWarningsBounded(warnings)})` : '';
  ...
  if (!matched) {
    ...
    // unreadable (partial write, corrupt JSON in a shared export dir) —
    // surface the skipped files so the caller can tell "typo'd id" from
    // "file failed to load". Warnings are basename-sanitized at the scan
    // boundary and the list is CAPPED (cubic P3): the error names the
    // first few, list_projects carries the full set. Only a genuine
    // readdir failure is fatal above.
    const detail = warnings.length
      ? `; unreadable files skipped: ${joinWarningsBounded(warnings)}`
      : '';
    return {
      project: null,
