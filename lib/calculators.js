    ...
    if (d) {
      ...
      if (!validPertOrdering(o, m, pe)) {
        issues.push({
          activityId: a.id,
          field: 'distribution',
          message: `pert estimates violate optimistic <= mostLikely <= pessimistic; using the 1-day sampling fallback`,
          received: {
...
export function runMonteCarlo({
  ...
  if (distributions && Object.keys(distributions).length > 0) {
    ...
    dists = {};
    const actById = new Map(safe.map((a) => [a.id, a]));
    for (const a of safe) {
      const spec = distributions[a.id];
      // Plain-object guard (cubic round-2): arrays and boxed primitives pass
      // `typeof === 'object'` but are not distribution specs.
      if (spec !== undefined && (spec === null || Array.isArray(spec) || typeof spec !== 'object')) {
        issues.push({
          activityId: a.id,
          field: 'distribution',
          message: 'distribution spec is not a distribution object; using the duration-derived default',
          received: spec ?? null
        });
      }
      if (spec === undefined || spec === null || Array.isArray(spec) || typeof spec !== 'object') {
        // Omitted or invalid spec: use the SAME duration-based default the
        // builder path derives, never the sampler's bare 1-day fallback
        // (CodeRabbit: a partially-supplied map silently understated every
        // activity it didn't mention).
        const base = validDuration(a.duration) ? a.duration : 1;
        dists[a.id] = {
          type: 'triangular',
          optimistic: base * 0.7,
