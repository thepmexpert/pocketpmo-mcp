...
export function buildDistributions(activities) {
  ...
  for (const activity of activities) {
    ...
    if (activity.distribution) {
      ...
      // default triple, not the bare duration (review #19).
      const oDefault = Math.max(1, base * 0.7);
...
export function evmMetrics({
  ...
  const inProgressFraction =
    Math.min(Math.max(effectiveInProgressValue, 0), 100) / 100;
  if (
    ...
    issues.push({
      ...
      message: 'inProgressValue out of range; clamped to [0, 100]',
  ...
  for (const m of safeMilestones) {
    ...
    // Normalize the known sentinels, then Number(); report unparseable.
    ...
    const pct = typeof m.percentage === 'number' ? m.percentage : Number(m.percentage);
    ...
    const rawCost = typeof m.cost === 'number' ? m.cost : Number(m.cost);
  ...
  let elapsedDays = 0;
  // Review #26: invalid dates used to yield NaN through getTime() and
  // poison PV/SPI silently. Every date is validated and reported now.
  ...
  if (startDate && !Number.isFinite(start)) {
    issues.push({ field: 'dates', message: 'invalid startDate', received: startDate });
  }
  if (endDate && !Number.isFinite(end)) {
    issues.push({ field: 'dates', message: 'invalid endDate', received: endDate });
  }
  if (statusDate && !Number.isFinite(status)) {
    issues.push({ field: 'dates', message: 'invalid statusDate', received: statusDate });
  }
  if (!startDate || !endDate) {
    ...
    // Review #27: date-only schedules use UTC calendar-day arithmetic —
    // plain-date strings ("2026-03-01") are parsed as UTC midnights, so
    // day counts are exact and immune to runtime-timezone DST shifts.
    // (The app's calculateEvmTimeline uses local-time Date parsing and
    // 86400000 division; this is a documented, more-correct divergence.)
    ...
    // Full timestamps (with time components) still work: the status
    // instant is clamped into [start, end] before the calendar-day diff.
