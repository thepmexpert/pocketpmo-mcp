...
function findCyclePaths(effPredecessors, unresolvedSet) {
  // Recursive DFS: depth is bounded by the longest predecessor chain inside
  // the unresolved set. Node's stack tolerates ~10k frames and every tool
  // handler call is wrapped in try/catch (server.js), so a pathological
  // >10k-activity chain degrades to a clean JSON-RPC error, not a crash.
  // Real project networks are orders of magnitude smaller; an iterative
  // rewrite would churn battle-tested cycle diagnostics for a theoretical
  // bound (deliberate scope call, review batch 6 finding 13).
