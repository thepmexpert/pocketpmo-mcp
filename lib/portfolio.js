...
export function createPortfolioFold({ statusDate } = {}) {
  // Resolve the default as-of date ONCE (cubic round 5): evmMetrics' own
  // per-call default would re-read the clock for every project, so a scan
  // crossing UTC midnight mixed as-of dates within one portfolio. One
  // fold == one as-of date, explicit or resolved-at-creation.
  const effectiveStatusDate =
