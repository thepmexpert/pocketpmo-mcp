 ...
 * All results rounded to 2dp; non-finite input -> NaN (never throws).
 */
...
export function pertRollup(activities) {
  const expected = activities.reduce((s, a) => s + (a.expected || 0), 0);
  const variance = activities.reduce((s, a) => s + (a.variance || 0), 0);
  return {
    expected: round2(expected),
    variance: round2(variance),
    stdDev: round2(Math.sqrt(variance))
 ...
 * Mirrors calculatePertCompletionProbability: invalid stdDev -> 0.5;
 * z clamped to [-3, 3]; zero variance -> deterministic 0/1.
 */
export function pertCompletionProbability(totalExpected, totalStdDev, targetDuration) {
  ...
  const z = Math.max(-3, Math.min(3, (target - expected) / stdDev));
  return round4(normalCdf(z));
}
 ...
 * order (same as the app). Unreachable activities (cycles) are reported in
...
export function cpmNetwork(activities) {
  if (!Array.isArray(activities) || activities.length === 0) {
    return { activities: [], projectDuration: 0, unresolved: [] };
  }
  const map = new Map(activities.map((a) => [a.id, a]));
  const done = new Set();
  const predsOf = new Map(activities.map((a) => [a.id, predecessorIds(a)]));
  ...
  let changed = true;
  while (changed && done.size < activities.length) {
    changed = false;
    for (const a of activities) {
      if (done.has(a.id)) continue;
      const preds = effPredecessors.get(a.id);
      const duration = typeof a.duration === 'number' && a.duration > 0 ? a.duration : 0;
      if (preds.length === 0) {
        a.es = 0;
        a.ef = duration;
        done.add(a.id);
  ...
  const unresolved = activities.filter((a) => !done.has(a.id)).map((a) => a.id);
  const processed = activities.filter((a) => done.has(a.id));
  ...
  for (const a of byEfDesc) {
    ...
    a.ls = a.lf - (typeof a.duration === 'number' && a.duration > 0 ? a.duration : 0);
    a.float = a.ls - a.es;
...
export function buildDistributions(activities) {
  const distributions = {};
  if (!Array.isArray(activities)) return distributions;
  for (const activity of activities) {
    if (!activity || !activity.id) continue;
    const base =
      typeof activity.duration === 'number' && activity.duration > 0 ? activity.duration : 1;
    if (activity.distribution) {
      const d = activity.distribution;
      distributions[activity.id] = {
        type: d.type || 'triangular',
        optimistic: pick(d.min, d.optimistic, Math.max(1, base * 0.7)),
        mostLikely: pick(d.mode, d.mostLikely, base),
        pessimistic: pick(d.max, d.pessimistic, base * 1.5),
        mean: d.mean !== undefined ? d.mean : base,
        stdDev: d.stdDev !== undefined ? d.stdDev : base * 0.2
      };
    } else {
      distributions[activity.id] = {
        type: 'triangular',
...
function sampleDuration(dist, rng) {
  if (dist.type === 'normal') {
    ...
    return Math.max(0, dist.mean + dist.stdDev * z);
  }
...
function validateActivities(activities) {
  ...
  const ids = new Set(activities.filter((a) => a && a.id != null).map((a) => a.id));
  for (const a of activities) {
    if (!a || a.id == null) {
...
export function runMonteCarlo({
  ...
  // Deterministic expected/stdDev for the target probability normal approx.
  const expectedTotal = safe.reduce((s, a) => {
    const d = dists[a.id];
    return s + (d ? (d.mean !== undefined ? d.mean : (d.optimistic + 4 * d.mostLikely + d.pessimistic) / 6) : 1);
  }, 0);
  const stdDevTotal = Math.sqrt(
    safe.reduce((s, a) => {
      const d = dists[a.id];
      if (!d) return s + Math.pow(1 / 6, 2);
      if (d.type === 'normal') return s + Math.pow(d.stdDev, 2);
      return s + Math.pow((d.pessimistic - d.optimistic) / 6, 2);
    }, 0)
  );

  const probabilityByTarget = (targets || []).map((t) => ({
    target: Number(t),
    probability: pertCompletionProbability(expectedTotal, stdDevTotal, Number(t))
  }));
