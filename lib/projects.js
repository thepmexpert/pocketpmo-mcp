...
// and size so an edited file is re-read; entries for files that disappear
// from the dir are evicted on the next listProjects scan.
...
function readViaFd(filePath) {
  ...
  try {
    ...
    // (open/perm failures included) runs BEFORE the cache is consulted, so a
    // recovered file (e.g. permissions fixed) is retried, never masked by a
    // stale failure entry.
    const cached = parseCache.get(filePath);
    if (cached && cached.mtimeMs === fst.mtimeMs && cached.size === fst.size) {
      return cached.entry;
    }
