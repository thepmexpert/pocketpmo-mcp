...
/**
 * Bridge-original diagnostics (NOT a port — the app schema has no
 * dependency type/lag fields to mirror): project export files are
 * hand-editable JSON, so a file CAN declare SS/FF dependency types or
 * lag on predecessor objects. The FS-only CPM pass drops those fields
 * silently, which mis-models the declared schedule with zero signal.
 * Contract: every declared-but-unsupported semantic surfaces as a
 * structured issue (warn-and-continue, bridge convention) on every path
 * that consumes the dependency network — validateActivities feeds both
 * critical_path and monte_carlo. The math itself is NEVER changed here;
 * declared semantics are reported as treated (FS, zero lag).
 */
export function dependencyTypeIssues(activities) {
  const issues = [];
  if (!Array.isArray(activities)) return issues;
  for (const a of activities) {
    if (!a || a.id == null || !Array.isArray(a.predecessors)) continue;
    for (const p of a.predecessors) {
      // Only object-form predecessors can carry type/lag; string entries
      // (and null/garbage entries, reported elsewhere) are not this check's
      // business — no double-reporting, no new issue classes.
      if (!p || typeof p !== 'object' || Array.isArray(p)) continue;
      if (typeof p.id !== 'string' || !p.id.trim()) continue;
      const typePresent = p.type !== undefined && p.type !== null;
      const lagPresent = p.lag !== undefined && p.lag !== null;
      const typeSupported =
        typePresent && typeof p.type === 'string' && p.type.trim().toUpperCase() === 'FS';
      const lagZero =
        lagPresent && typeof p.lag === 'number' && Number.isFinite(p.lag) && p.lag === 0;
      if ((typePresent && !typeSupported) || (lagPresent && !lagZero)) {
        const declared = [];
        if (typePresent && !typeSupported) declared.push(`type ${JSON.stringify(p.type)}`);
        if (lagPresent && !lagZero) declared.push(`lag ${JSON.stringify(p.lag)}`);
        issues.push({
          activityId: a.id,
          field: 'predecessors',
          message: `predecessor '${p.id}' declares ${declared.join(' and ')}; only finish-to-start (FS) with zero lag is supported — treated as FS with zero lag`,
          received: { type: p.type ?? null, lag: p.lag ?? null }
        });
      }
    }
  }
...
/**
 * The EFFECTIVE PERT triple for an activity: declared distribution fields
 * with the duration-derived fallbacks the pert tool path has always used.
 * Single source of truth for the derivation — the server pert handler and
 * the portfolio roll-up must never grow second, drifting copies of it.
 */
export function effectivePertTriple(a) {
  return {
    o: a?.distribution?.optimistic ?? (a?.duration ?? NaN) * 0.7,
    m: a?.distribution?.mostLikely ?? a?.duration ?? NaN,
    pe: a?.distribution?.pessimistic ?? (a?.duration ?? NaN) * 1.5
  };
}
...
export function validateActivities(activities) {
  ...
  // Declared-but-unsupported dependency semantics (SS/FF/lag) on hand-edited
  // files: report, never silently mis-model. Lives in the SHARED validator
  // so every network-consuming path (critical_path, monte_carlo) inherits
  // it — a fix inside one handler does not guard sibling call paths.
  issues.push(...dependencyTypeIssues(activities));
  return issues;
}
...
/** Default event-loop yield cadence for runMonteCarloAsync (iterations). */
...
export async function runMonteCarloAsync({
  ...
  // disable every event-loop yield (i % 0 is NaN) and starve the event loop
  // the whole feature exists to protect — normalize to the default instead
  // (bot sweep round 1, cubic P2). Single named constant: the destructure
  // default and the fallback must never drift apart (bot sweep round 2).
  const yieldStep =
    Number.isSafeInteger(yieldEvery) && yieldEvery >= 1
      ? yieldEvery
      : DEFAULT_YIELD_EVERY;
...
// Round to 4 decimal places (NaN-safe). Exported for the portfolio roll-up,
// which computes portfolio CPI/SPI at the same precision evmMetrics uses.
