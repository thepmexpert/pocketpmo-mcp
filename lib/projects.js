...
// itself would force re-reads. Memory ceiling = entries x MAX_FILE_SIZE in
// the absolute worst case; realistic exports are a few KB each.
...
//   hits      — reads served from cache (no file access)
...
//               oversized): re-verified on every scan by design, never cached
//   evictions — entries dropped (vanished files + FIFO cap overflow)
 ...
 * the projects dir. Returns true if an entry was dropped. */
export function invalidate(filePath) {
  const abs = path.isAbsolute(filePath)
    ? filePath
    : path.join(projectsDir(), filePath);
  return parseCache.delete(abs);
}
