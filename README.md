...
> test-runner involvement). The **test suite** needs Node ≥ 20.11: it uses
> `node:test`'s `describe`/`it` (stable from Node 20) plus MockTimers via the
> options-object `enable()` and `setTime()`, both added in Node v20.11.0.
