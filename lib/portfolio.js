...
/**
 * Roll up PERT and EVM across a set of already-loaded project objects.
 * Pure: no IO, no env, deterministic for a given statusDate. Fail-soft per
 * dimension — a project with no activities or no EVM data is skipped WITH
 * a reason on its row, never silently dropped, never zero-filled.
 *
...
export function portfolioRollup(projects, { statusDate } = {}) {
  ...
  const pert = pertProjects === 0 ? null : {
    ...
    skippedActivities: rows
      ...
      .map((r) => ({ project: r.id, ids: r.pert.skipped })),
    skippedProjects: rows
      .filter((r) => r.pert === null)
      .map((r) => ({ project: r.id, reason: r.pertSkipReason }))
  };
  ...
  const evm = evmProjects === 0 ? null : {
    ...
    skippedProjects: rows
      .filter((r) => r.evm === null)
      .map((r) => ({ project: r.id, reason: r.evmSkipReason }))
  };

  // Fresh copy per response: a caller mutating the returned array must not
  // poison the assumptions of every later roll-up (pure-function isolation).
  return { pert, evm, projects: rows, assumptions: [...ASSUMPTIONS] };
}
