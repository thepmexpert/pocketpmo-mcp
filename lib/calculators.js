...
export function runMonteCarlo({
  ...
  if (distributions && Object.keys(distributions).length > 0) {
    ...
    dists = {};
    for (const [id, spec] of Object.entries(distributions)) {
      if (!spec || typeof spec !== 'object') {
        issues.push({
          activityId: id,
          field: 'distribution',
          message: 'distribution spec is not an object; activity samples the 1-day fallback',
          received: spec ?? null
        });
        continue;
      }
      const base = validDuration(safe.find((a) => a && a.id === id)?.duration)
        ? safe.find((a) => a && a.id === id).duration
        : 1;
      dists[id] = normalizeDistributionSpec(id, spec, base, issues);
    }
  } else {
