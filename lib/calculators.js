...
export function evmMetrics({
  ...
  // Round-trip check: rejects impossible dates ('2026-02-30') that native
  // parsing would silently normalize.
  const calendarDayEq = (v, t) => {
    const d = new Date(t);
    return (
      d.getUTCFullYear() === Number(v.slice(0, 4)) &&
      d.getUTCMonth() + 1 === Number(v.slice(5, 7)) &&
      d.getUTCDate() === Number(v.slice(8, 10))
    );
  };
  ...
  const checkDate = (field, value, t) => {
    ...
    } else if (isDateOnly(value) && !calendarDayEq(value, t)) {
      issues.push({ field: 'dates', message: `invalid ${field} (impossible calendar date)`, received: value });
    }
  };
