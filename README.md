...
| `pert_estimate` | `project`, `targetDuration?` | per-activity expected/variance/stdDev, roll-up, P(completion ≤ target). Assumes independent activity durations — correlated durations (shared resources, common risks) make the true variance larger; treat roll-up as optimistic then |
