...
// the scan. Unconvertible values get a stable placeholder — they stay
// listed but simply cannot match.
...
export function listProjects() {
  ...
  forEachEntry((item, err) => {
    ...
    result.projects.push({
      ...
      name: project.name || safeString(project.id),
...
export function getProject(idOrName) {
  ...
  forEachEntry((item, err) => {
    ...
    const project = item.entry.project;
    validIds.push(safeString(project.id));
    ...
    // dir (defect pre-existing since v0.1; cubic round 3).
    const name = safeString(project.name || project.id);
    if (
      safeString(project.id).toLowerCase() === key ||
